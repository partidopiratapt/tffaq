...
/***	add new category or update an existing one	***/

// TODO: currently a category name that already exists can be added causing multiple categories with the same name. needs to be fixed
...
/*** do not allow an empty category to be entered	***/
...
}		/***	end add new category or update an existing one	***/
...
}	/***	end if(isset($_GET['delete_cat']))	***/

/***	end delete a single category	***/
...
/***	do they want to delete the categories?	***/
...
/***	does the category have questions?	***/
...
$questions = faq_get_questions(absint($category));
 ...
 if(empty($questions)) {
	...
	/*** because we're doing multiples we'll break and throw a message on any error	***/
	if(!$status == 1) { break;}
	...
	}	/***	end foreach($selected as $category)	***/
...
}	/*** end if(isset($_POST['tffaq_category']))	***/

else {	/***	throw status message because no checkboxes were selected	***/
...
/***	end delete multiple categories	***/
...
/*** have not added status messages here but am considering it	***/

/*** end sort the categories	***/
...
/***	confirm all questions are in a valid category	***/

tffaq_category_validation();

/*** if a message it set let's show it here	***/
...
/*** end new category meta box	***/

/*** edit category meta box	***/
