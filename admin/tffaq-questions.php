...
//	wrap the whole thing in if(current_user_can('administrator'))
...
/*** do not allow an empty question to be added	***/
	...
	/***** update and send the email notification *****/
...
/*** email notify but only if email is not empty	***/
...
wp_mail($email,$subject,$email_body,$headers);	/*** add a status message here?	***/
...
/*** make sure it returns to the correct page	***/

$_GET['cat'] = $category;

}

/***	end add or update the question ***/

/***	delete a question	***/

/***	all ready asked if you were sure with javascript	***/
...
$check_admin_referer = check_admin_referer('tffaq_sort', '_wpnonce');
	...
	}	/***	end if($check_admin_referer == 1 ) ***/

}	/***	end if(isset($_GET['direction']))	***/

/*** end sort the questions	***/


/***	start process the checkboxes	***/
...
if(isset($_POST['tffaq_question'])) {
	...
	/***	do they want to delete the questions?	***/
...
if(($action == 'delete') and ($_POST['tffaq_confirm'] == 'true')) {
	...
	/*** because we're doing multiples we'll beak and throw a message on any error	***/
			if(!$status == 1) { break;}

			}	/*** end foreach($question as $key => $value)	***/
			}	/*** end foreach($selected as $question)	***/
	...
	} else {	/***	end of delete, start of move questions to different category	***/

	/*** move the selected questions to a different category but only if it was not a canceled delete	***/
...
if($action != 'delete') {
	...
	}	/***	end else move the selected questions to a different category	***/

} else {	/***	end if(isset($_POST['tffaq_question ']))	***/

	/*** if they got to here they submitted the form without selecting any checkboxes so throw an error message	***/

	$_POST['tffaq_message'] = array('class' => 'error', 'status' => __('There was an Error! No Questions were Selected')) ;

	}

}	/***	end if(isset($_POST['tffaq_checkbox_submit'])	***/


/***	end process the checkboxes	***/

/***	end process the forms	***/
...
/*** if a message is set let's show it here	***/
...
<?php }	/***	end if(current_user_can('administrator'))	***/